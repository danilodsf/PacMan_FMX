...
uses
  ...
  System.SysUtils,
  System.UITypes,
  uSettings, System.Math, FMX.Forms;
...
type
  ...
  private
    ...
    FSettings: TSettings;
  public
    property Map: TArray<TArray<Char>> read FMap write FMap;
...
constructor TBoard.Create(ASettings: TSettings);
begin
  FSettings := ASettings;

  font := TFont.Create;
  font.Family := 'Courier New';
  font.Size := Round(FSettings.Scale * 1.5);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        rect := RectF(x * FSettings.Scale, y * FSettings.Scale, (x + 1) * FSettings.Scale, (y + 1) * FSettings.Scale);
      ...
      begin
        ...
        rect := RectF((x * FSettings.Scale) - (FSettings.Scale / 2), (y * FSettings.Scale) - (FSettings.Scale / 2),
                      (x * FSettings.Scale) + (FSettings.Scale * 1.5) - (FSettings.Scale / 2), (y * FSettings.Scale) + (FSettings.Scale * 1.5) - (FSettings.Scale / 2));
        ACanvas.FillRect(rect, 0, 0, AllCorners, 1);
      end;
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ACanvas.Fill.Color := TAlphaColors.White;
        ACanvas.FillEllipse(RectF((x * FSettings.Scale) + (FSettings.Scale / 4) - (FSettings.Scale / 5), (y * FSettings.Scale) + (FSettings.Scale / 4) - (FSettings.Scale / 5),
                                 (x * FSettings.Scale) + (FSettings.Scale / 4) + (FSettings.Scale / 5), (y * FSettings.Scale) + (FSettings.Scale / 4) + (FSettings.Scale / 5)), 1);
      end
      else if FMap[y, x] = 'o' then
      begin
        ACanvas.Fill.Color := TAlphaColors.White;
        ACanvas.FillEllipse(RectF((x * FSettings.Scale) + (FSettings.Scale / 4) - (FSettings.Scale / 2), (y * FSettings.Scale) + (FSettings.Scale / 4) - (FSettings.Scale / 2),
                                 (x * FSettings.Scale) + (FSettings.Scale / 4) + (FSettings.Scale / 2), (y * FSettings.Scale) + (FSettings.Scale / 4) + (FSettings.Scale / 2)), 1);
      end;
    end;
  end;
end;
...
begin
  ...
  score_text := Format('Score: %d', [FSettings.Score]);
  lives_text := Format('Lives: %dX', [Max(FSettings.Lives, 0)]);
  ...
  y_score_pos := FSettings.Scale * 30.75;
  ...
  y_lives_pos := FSettings.Scale * 32;
  ...
  begin
    ...
    y_end_pos := FSettings.Scale * 12.25;
    ...
    y_game_pos := FSettings.Scale * 13.75;
